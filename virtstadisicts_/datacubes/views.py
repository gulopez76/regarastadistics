...
def WriteToExcel(weather_data):
    ...
    worksheet_s.write(4, 6, u"Entrega Parcial", header)
    worksheet_s.write(4, 7, u"Entrega Total", header)
    worksheet_s.write(4, 8, u"Nominal", header)
    worksheet_s.write(4, 9, u"Total Magatzem", header)
    worksheet_s.write(4, 10, u"Dt.Expedició", header)
    worksheet_s.write(4, 11, u"Transportista", header)
    worksheet_s.write(4, 12, u"Localitazació", header)
    worksheet_s.write(4, 13, u"Màquina", header)
    worksheet_s.write(4, 14, u"Agent", header)
    worksheet_s.write(4, 15, u"Dt.Fi", header)
    for idx, data in enumerate(weather_data):
	    ...
	    worksheet_s.write_number(row, 6, data.totaldeliver, field)
	    worksheet_s.write_number(row, 7, data.deliverparcial, field)
	    worksheet_s.write_number(row, 8, data.nominalprovided, field)
	    worksheet_s.write_number(row, 9, data.totalstorage, field)
	    try:
	    	nt2 = data.delivereddate.strftime('%d/%m/%Y')
	    	if data.delivereddate is not None:
	       		nativedt2 = data.delivereddate.replace(tzinfo=None)
	       		worksheet_s.write_datetime(row, 10, nativedt2, date_time_format)
	    	else:
	      		nativedt2 = ''
	       		worksheet_s.write_string(row, 10, nativedt2, field)
	    except:
	        nativedt2 = ''
	       	worksheet_s.write_string(row, 10, nativedt2, field)
	    worksheet_s.write_string(row, 11, data.idcarrier.carriername, field)
	    worksheet_s.write_string(row, 12, data.location, field)
	    ...
	    try:
	    	...
	    	if data.enddate is not None:
	       		...
	       		worksheet_s.write_string(row, 15, nt2, field)
	    	else:
	      		nativedt2 = ''
	       		worksheet_s.write_string(row, 15, nativedt2, field)
	    except:
	        nativedt2 = ''
	       	worksheet_s.write_string(row, 15, nativedt2, field)
    workbook.close()
...
#  	      detail = ots_deliver.objects.order_by('sentdate','ot','clientname').all().filter(sentdate__range=[str(dfini), str(dffi)], ot = actualot)
	      detail = ots_deliver.objects.select_related('idcarrier').order_by('sentdate','ot','clientname').all().filter(sentdate__range=[str(dfini), str(dffi)], ot = actualot)
...
#    	      detail = ots_deliver.objects.order_by('sentdate','ot','clientname').all().filter(sentdate__range=[str(dfini), str(dffi)], clientname = actualcli)
    	      detail = ots_deliver.objects.select_related('idcarrier').order_by('sentdate','ot','clientname').all().filter(sentdate__range=[str(dfini), str(dffi)], clientname = actualcli)
	    elif actualcli <> "Tots" and actualot <> "Totes":
 #   	      detail = ots_deliver.objects.order_by('sentdate','ot','clientname').all().filter(sentdate__range=[str(dfini), str(dffi)], clientname = actualcli, ot = actualot)
    	      detail = ots_deliver.objects.select_related('idcarrier').order_by('sentdate','ot','clientname').all().filter(sentdate__range=[str(dfini), str(dffi)], clientname = actualcli, ot = actualot)
	    else:
#    	      detail = ots_deliver.objects.order_by('sentdate','ot','clientname').all().filter(sentdate__range=[str(dfini), str(dffi)])
	   ...
	   'rows': rows,
		...
		for ix in range(0,len(ottmp)):
			print "======================================================================================="	    
			print "===				     DICT VALUES TO SAVE   		           ==="
			print "===				Value ID --->   ", idtmp[ix],"			   ==="
			print "===				Value OT --->   ", ottmp[ix],"			   ==="
			print "===				Value COD PRODUCT --->   ", prodtmp[ix],"	   ==="
			print "===				Value COMMAND CLI --->  ", comtmp[ix],"	   	   ==="
			print "===				Value DELIVERED DATE ---> ",dtdeltmp[ix],"   	   ==="
			print "===				Value STORAGE --->   ", storagetmp[ix],"	   ==="
			print "===				Value CARRIER --->   ", carriertmp[ix],"	   ==="
			print "======================================================================================="	    
			up_id = idtmp[ix]
...
#			up_storage = storagetmp[ix]
			print up_id
			print up_deldt
			print up_car
#			print up_storage
#			ots_deliver.objects.filter(pk=up_id).update(totalstorage=up_storage, idcarrier = up_car, delivereddate = up_deldt)
...
#    otdataini = ots.objects.order_by('ot').all()
	...
	if request.POST['action'] == "apply":
	    ...
	    print dffi
	    if actualcli == "Tots" and actualot <> "Totes":
    	      detail = ots_deliver.objects.select_related('idcarrier').order_by('sentdate','clientname','ot').all().filter(sentdate__range=[str(dfini), str(dffi)], ot = actualot)
	    elif actualcli <> "Tots" and actualot == "Totes":
    	      detail = ots_deliver.objects.select_related('idcarrier').order_by('sentdate','clientname','ot').all().filter(sentdate__range=[str(dfini), str(dffi)], clientname = actualcli)
	    elif actualcli <> "Tots" and actualot <> "Totes":
    	      detail = ots_deliver.objects.select_related('idcarrier').order_by('sentdate','clientname','ot').all().filter(sentdate__range=[str(dfini), str(dffi)], clientname = actualcli, ot = actualot)
	    else:
